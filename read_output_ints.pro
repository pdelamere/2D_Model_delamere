...
val = 0.0
close,1
fil = './plots/run1/'+filetype+spec+nt+'_3D.dat'
...
for i = 0,200,5 do begin
   w = window(window_title='torus',dimensions=[xsz,ysz],margin=0,$
              buffer=1)  
   nfrm = i
   read_data,nfrm,nlng,nr,'DENS','elec',dens_elec,xarr,yarr,xar,yar
   read_data,nfrm,nlng,nr,'DENS','sp',dens_sp,xarr,yarr,xar,yar
   read_data,nfrm,nlng,nr,'TEMP','sp',temp_sp,xarr,yarr,xar,yar
   arr = dens_elec*dens_sp/temp_sp
